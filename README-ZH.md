...
将 `event-lead-cli/` 文件夹复制到你电脑上的任意位置，工具内部使用相对路径，位置不影响运行。
...
将活动收集到的所有 CSV 和 Excel 文件放入 `data/` 目录。
...
data_dir: "../data"             # 数据文件目录，相对于此配置文件的路径

sources:
  google_form:
    file: "your-form-export.csv"  # data_dir 内的文件名
    type: csv
    encoding: utf-8
    mapping:
      name: "姓名"                 # 填写 CSV 文件中实际的列名
      email: "Email"
      company_title: "公司名称 + 职位"
    survey_fields:
      interest_scenario: "感兴趣的 AI 应用场景"
      demo_interest: "是否希望安排产品演示"
      project_timeline: "项目预计时程"
